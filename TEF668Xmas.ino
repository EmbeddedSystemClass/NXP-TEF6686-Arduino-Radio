...
#define DATA_TIMER_INTERVAL 24
#define SIGNAL_SAMPLE_COUNT 3
byte DSP_I2C = 0x64;

int8_t radio_mode;
int8_t scan_radio_mode;
...
static const uint8_t DSP_INIT[] PROGMEM =
{
  ...
  0x05, 0x20, 0x56, 0x01, 0x03, 0xE8,                                                                          // FM_Set_Bandwidth_Options    (1, 1000)
  0x07, 0x30, 0x15, 0x01, 0x00, 0x80, 0x00, 0x01,                                                              // FM_Set_Ana_Out              (1, 128, 1)
};
...
void scan(bool continous)
{
  ...
  do
  {
    ...
    {
      Set_Cmd( scan_radio_mode == 0 ? 32 : 33, 1, 2, 1, freq);
      Serial.print(scan_radio_mode == 0 ? freq * 10 : freq, DEC);
      Serial.print('=');
      delay(10);

      int16_t uQuality[4] = { 0 };
      Get_Cmd(scan_radio_mode == 0 ? 32 : 33, 128, uQuality, 4);
...
void Set_AGC_treshold(uint8_t val)
{
  if (val == 0) {
    ...
    Set_Cmd(32, 11, 2, 860, 0);
    Set_Cmd(33, 11, 1, 900);
  } else {
    Set_Cmd(32, 11, 2, 830, 0);
    Set_Cmd(33, 11, 1, 840);
  }
}
    ...
    if (buff[buff_pos] != '\n' && buff_pos != SERIAL_BUFFER_SIZE - 1)
      ...
      switch (buff[0])
      {
        ...
        */
        ...
        case 'A':  // AGC
          Serial.print('A');
          AGC_treshold = atoi(buff + 1);
          Set_AGC_treshold(AGC_treshold);
          Serial.println(AGC_treshold);
          break;
